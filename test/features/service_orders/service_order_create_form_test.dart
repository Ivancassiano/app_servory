...
void main() {
  Widget host() => ProviderScope(
    overrides: [
      ...
      itemListProvider.overrideWith((ref) => Stream.value(const <LocalItem>[])),
      itemTypeListProvider.overrideWith(
        (ref) => Stream.value(const <LocalItemType>[]),
      ),
      for (final k in ReferenceKind.values)
        referenceListProvider(
          k,
        ).overrideWith((ref) => Stream.value(const <ReferenceItem>[])),
    ],
    child: const MaterialApp(
      home: ServiceOrderDetailScreen(serviceOrderId: 'new'),
    ),
  );

  testWidgets(
    'criação: sem switch "Abrir imediatamente"; botões Iniciar + Rascunho',
    (tester) async {
      ...
      expect(
        find.widgetWithText(FilledButton, 'Iniciar ordem'),
        findsOneWidget,
      );
      expect(find.widgetWithText(FilledButton, 'Agendar ordem'), findsNothing);
      expect(
        find.widgetWithText(TextButton, 'Salvar rascunho'),
        findsOneWidget,
      );
    },
  );
